...
[OrderedVectsProp] = CalculACP(VT);
W = OrderedVectsProp(:, 1:d);

Projected = Projection(W, VT);
ProjectedC1 = Projection(W, C1);
ProjectedC2 = Projection(W, C2);

% scatter(C1(:, 3), C1(:,2))
% scatter(C2(:, 3), C2(:,2))

pointsACP = droite2DVd(W, x);
plot(x, pointsACP, 'r');


%%%% ACI %%%%
[VectProp] = CalculACI(C1, C2);
pointsACI = droite2DVd(VectProp, x);
scatter(C1(:, 3), C1(:,2));
scatter(C2(:, 3), C2(:,2));
plot(x, pointsACI, 'g');
